...
import { setViewMode, setSearchQuery, setFilterLanguage } from "@/store/slices/projectsSlice"
...
const languages = ["All", "TypeScript", "JavaScript", "Python", "Go", "Java", "C++"]

export default function ProjectsPage() {
  const { viewMode, searchQuery, filterLanguage } = useSelector((state: RootState) => state.projects)
  ...
  const filteredProjects = mockProjects.filter((project) => {
    ...
    const matchesLanguage = filterLanguage === "" || filterLanguage === "All" || project.language === filterLanguage
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <CardDescription className="mt-1 text-gray-600 dark:text-gray-400">{project.description}</CardDescription>
          </div>
          <DropdownMenu>
      ...
      <CardContent>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{project.language}</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className={`w-2 h-2 rounded-full ${getStatusColor(project.status)}`} />
            <span className="text-xs text-gray-500 dark:text-gray-400 capitalize">{project.status}</span>
          </div>
        </div>
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4 flex-1">
            <div className={`w-4 h-4 rounded-full ${getLanguageColor(project.language)}`} />
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage your code projects and repositories</p>
        </div>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="flex flex-1 items-center space-x-4">
              ...
              <Select value={filterLanguage} onValueChange={(value) => dispatch(setFilterLanguage(value))}>
                <SelectTrigger className="w-40">
                  <SelectValue placeholder="Language" />
                </SelectTrigger>
                <SelectContent>
                  {languages.map((lang) => (
                    <SelectItem key={lang} value={lang === "All" ? "" : lang}>
                      {lang}
                    </SelectItem>
                  ))}
                </SelectContent>
